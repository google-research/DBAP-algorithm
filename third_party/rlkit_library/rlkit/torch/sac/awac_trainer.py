...
class AWACTrainer(TorchTrainer):
    def __init__(
        ...
        self.rnd_lr = rnd_lr
        self.rnd_net1 = rnd_net1 # Thing being trained
        self.rnd_net2 = rnd_net2 # Thing being matched
        self.rnd_optimizer = optimizer_class(
            self.rnd_net1.parameters(),
            weight_decay=rnd_weight_decay,
            lr=self.rnd_lr,
        )
        self.rnd_update_period = rnd_update_period
        self.policy = policy
        self.qf1 = qf1
        self.qf2 = qf2
        self._hindsight_relabel = hindsight_relabel
        self._stride_goal = stride_goal
        ...
        self.goal_optimizer = optimizer_class(
            self.goal_model.parameters(),
            weight_decay=policy_weight_decay,
            lr=policy_lr,
        )
        self.optimizers[self.goal_model] = self.goal_optimizer
        ...
        self.bc_goal_num_pretrain_steps = bc_goal_num_pretrain_steps
    ...
    def run_bc_batch_goal(self, replay_buffer, policy):
        batch = self.get_batch_from_buffer(replay_buffer, self.bc_batch_size)
        o_size = batch["observations"].shape[-1] // 2
        o = batch["observations"][:, :o_size]
        u = batch["observations"][:, o_size:]
        mean_val = torch.from_numpy(np.array([4.70130981, 3.92914984, 0.54396669, 0.12922642, -0.08235331,
               0.91269478, -0.21731469, 0.65739334, -0.20578889, 0.64921875,
               0.02841821])).cuda()
        std_val = torch.from_numpy(np.array([3.18300962e-01, 1.51710701e+00, 6.33534037e-01, 1.56521327e-01,
                           3.22179324e-01, 1.83723560e-01, 3.67900363e-01, 6.76053045e-02,
                           3.54140755e-01, 6.20011661e-02, 2.00769962e-02])).cuda()
        u = (u - mean_val)/(std_val)
    ...
    def pretrain_goalproposer_with_bc(self, goal_model, train_buffer, test_buffer, steps, label="policy", ):
        logger.remove_tabular_output(
            'progress.csv', relative_to_snapshot_dir=True,
        )
        logger.add_tabular_output(
            'pretrain_goal_%s.csv' % label, relative_to_snapshot_dir=True,
        )
        optimizer = self.optimizers[goal_model]
        prev_time = time.time()
        for i in range(steps):
            train_policy_loss, train_logp_loss, train_mse_loss, train_stats = self.run_bc_batch_goal(train_buffer, goal_model)
            ...
            test_policy_loss, test_logp_loss, test_mse_loss, test_stats = self.run_bc_batch_goal(test_buffer, goal_model)
            test_policy_loss = test_policy_loss

            if i % self.pretraining_logging_period==0:
                stats = {
                "pretrain_bc_goal/batch": i,
                "pretrain_bc_goal/Train Logprob Loss": ptu.get_numpy(train_logp_loss),
                "pretrain_bc_goal/Test Logprob Loss": ptu.get_numpy(test_logp_loss),
                "pretrain_bc_goal/Train MSE": ptu.get_numpy(train_mse_loss),
                "pretrain_bc_goal/Test MSE": ptu.get_numpy(test_mse_loss),
                "pretrain_bc_goal/train_policy_loss": ptu.get_numpy(train_policy_loss),
                "pretrain_bc_goal/test_policy_loss": ptu.get_numpy(test_policy_loss),
                "pretrain_bc_goal/epoch_time":time.time()-prev_time,
                }

                logger.record_dict(stats)
                logger.dump_tabular(with_prefix=True, with_timestamp=False)
                pickle.dump(self.goal_model, open(logger.get_snapshot_dir() + '/bc_%s.pkl' % label, "wb"))
                prev_time = time.time()
    ...
    # Do hindsight relabeling
    def path_process(self, paths, replay_buffer):
        if self._hindsight_relabel:
            print("Going to do relabeling")
            for path in paths:
                num_obs = len(path['observations'])
                for i in range(0, num_obs - 1, self._stride_goal):
                    for j in range(i + 1, num_obs, self._stride_goal):
                        o = path['observations'][i].copy()
                        no = path['next_observations'][i].copy()
                        ac = path['actions'][i].copy()
                        r = path['rewards'][i].copy()
                        if (i + 1) == j: # If it's the step before the goal, then reward is 1.0
                            r = 1.0
                        else:
                            r = 0.0
                        ei = path['env_infos'][i].copy()
                        terminal = path['terminals'][i].copy()

                        g = path['observations'][j].copy()
                        o_size = int(g.shape[0] // 2)
                        o[o_size:] = g[:o_size].copy()
                        no[o_size:] = g[:o_size].copy()

                        replay_buffer.add_sample(o, ac, r, terminal, no, env_info=ei)
        else:
            print("No relabeling")
    ...
    def eval_rnd_inv_densities(self, batch):
        o_size = batch['observations'].shape[1] // 2
        obs = torch.from_numpy(batch['observations'][:, :o_size]).float().to(ptu.device)
        rnd1_pred = self.rnd_net1(obs)
        rnd2_pred = self.rnd_net2(obs)

        # RND reward
        rnd_loss = self.rnd_criterion(rnd1_pred, rnd2_pred.detach())
        rnd_rewards = ptu.get_numpy(rnd_loss)
        return rnd_rewards
    ...
    def train_from_torch(self, batch, train=True, pretrain=False,):
        ...
        o_size = obs.shape[1] // 2
        # rnd1_pred = self.rnd_net1(obs[:, :o_size])
        # rnd2_pred = self.rnd_net2(obs[:, :o_size])

        # RND reward
        # rnd_loss = self.rnd_criterion(rnd1_pred, rnd2_pred.detach())
        # if self._n_train_steps_total % self.rnd_update_period == 0:
        ...
        if self._need_to_update_eval_statistics:
            ...
            # self.eval_statistics['RND Loss'] = np.mean(ptu.get_numpy(rnd_loss))
    ...
    def visualize_2d_vf(self):
        xo = np.arange(-1, 1, 0.05)
        yo = np.arange(-1, 1, 0.05)
        xx, yy = np.meshgrid(xo, yo) # State meshgrid
        x = np.arange(-1, 1, 0.2)
        y = np.arange(-1, 1, 0.2)
        VFs = []
        # For each goal generate the full meshgrid of values.
        for xg in x:
            for yg in y:
                xx_flat = xx.reshape(-1)
                yy_flat = yy.reshape(-1)
                xxg_flat = np.repeat(xg, len(xx_flat))
                yyg_flat = np.repeat(yg, len(yy_flat))
                obs_concat = np.concatenate([xx_flat[:, None],
                                             yy_flat[:, None],
                                             xxg_flat[:, None],
                                             yyg_flat[:, None]], axis=-1)
                obs_concat = torch.from_numpy(obs_concat).float().to(ptu.device)
                vf = self.compute_vf(obs_concat)
                vf = vf.detach().cpu().numpy()
                vf = vf.reshape(len(xo), len(yo))
                VFs.append(vf)
        return VFs
    ...
    def end_epoch(self, epoch):
        ...
        # TODO: Visualize all the VFs
        # VFs = self.visualize_2d_vf()
        # os.makedirs('epoch_%d_vfs'%epoch, exist_ok = True)
        # for i, vf in enumerate(VFs):
        #     plt.clf()
        #     plt.cla()
        #     plt.imshow(vf)
        #     plt.savefig('epoch_%d_vfs/goal_vf_%d.png'%(epoch, i))
        self._need_to_update_eval_statistics = True
